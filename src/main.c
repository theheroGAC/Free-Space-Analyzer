...
typedef enum { F_ALL=0, F_GAMES, F_MP3, F_OGG, F_JPG, F__COUNT } Filter;
...
int main(int argc, char* argv[]) {
    ...
    int running = 1, move_delay = 0, calculating = 0, exiting=0;
    ...
    while(running){
        ...
        // exit
        if(!exiting && (pressed & SCE_CTRL_CROSS)) { exiting=1; calculating=0; }
        ...
        // analog stick per cambiare partizione
        if(!exiting && move_delay>0) move_delay--;
        int prev_part=current_part;
        if(!exiting && move_delay==0){
            if(pad.ly<128-STICK_THRESHOLD){ 
                ...
                calculating=1; 
                sceRtcGetCurrentTick(&last_switch_time);
                current_folder=0; // reset cartella
            }
            if(pad.ly>128+STICK_THRESHOLD){ 
        ...
        frame_count++; if(frame_count>=30){ frame_count=0; fps=30;}

        // elapsed
        SceRtcTick now; sceRtcGetCurrentTick(&now);
        ...
        // draw
        ui_draw(parts, parts_count, current_part, top, top_count, battery, fps, calculating?1.0f:0.0f, current_folder);

        old_pad = pad;

        if(exiting){ vita2d_wait_rendering_done(); running=0; }
    }
